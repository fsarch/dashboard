...
const LayerGeometryPanel: React.FunctionComponent<LayerGeometryPanelProps> = ({ geometry, hasSize, onChange }) => {
  ...
  return (
    <Section name="Position, Größe & Rotation" color={colors.lightPurple}>
      <FieldsetRow label="X">
        <input type="number" value={Math.round(geometry.x)} onChange={(e) => handleFieldChange('x', e.target.value)} />
      </FieldsetRow>
      <FieldsetRow label="Y">
        <input type="number" value={Math.round(geometry.y)} onChange={(e) => handleFieldChange('y', e.target.value)} />
      </FieldsetRow>
      <FieldsetRow label="Rotation (°)">
        <input type="number" value={Math.round(geometry.rotationDeg)} onChange={(e) => handleFieldChange('rotationDeg', e.target.value)} />
      </FieldsetRow>
      {hasSize && (
        <>
          <FieldsetRow label="Breite">
            <input type="number" min={1} value={Math.round(geometry.width)} onChange={(e) => handleFieldChange('width', e.target.value)} />
          </FieldsetRow>
          <FieldsetRow label="Höhe">
            <input type="number" min={1} value={Math.round(geometry.height)} onChange={(e) => handleFieldChange('height', e.target.value)} />
          </FieldsetRow>
        </>
      )}
    </Section>
