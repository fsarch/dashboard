...
import SchemaVersionForm from '@/components/apps/dblight/SchemaVersionForm.component';

export const generateMetadata = createAutomaticMetadata();

type SchemaVersionCreatePageProps = {
  params: Promise<{ serviceId: string; collectionId: string }>;
};

export default async function SchemaVersionCreatePage({ params }: SchemaVersionCreatePageProps) {
  ...
  return (
    <DefaultPage>
      <Section name={`Schema-Version anlegen: ${collection.name}`}>
        <SchemaVersionForm
          serviceId={serviceId}
          collectionId={collectionId}
          currentSchema={activeVersion?.schema ?? {}}
        />
      </Section>
    </DefaultPage>
  );
