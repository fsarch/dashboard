...
import { TAggregateMeasurementsDto, TAggregateResult } from '@/services/metric-server/metric-server.type';
...
export async function aggregateMeasurementsAction(
  ...
  try {
    ...
    const result = await metricServerService.aggregateMeasurements(
      metricId,
      dto,
      serviceId
    );
