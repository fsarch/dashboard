...
import { TPaginationResultDto, TScopeDto } from '@/services/credence/credence.type';
...
const ScopesList: React.FunctionComponent<ScopesListProps> = ({
  ...
  const handlePageChange = (newPage: number) => {
    router.push(`/credence/${serviceId}/scope?page=${newPage}&pageSize=${pageSize}`);
  };

  const handlePageSizeChange = (newPageSize: number) => {
    router.push(`/credence/${serviceId}/scope?page=1&pageSize=${newPageSize}`);
  };
        ...
        <>
          <List>
            {data.map((scope) => (
              <Link
                key={scope.id}
                href={`/credence/${serviceId}/scope/${scope.id}`}
              >
                <ListItem>
